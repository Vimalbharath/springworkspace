...
//		return false;
//	}
	public boolean editMatchdetails2(int detailsid, Details newdetails ) {
	    Optional<Details> optDetails = detailsRepository.findById(detailsid);
	    ...
	    if (optDetails.isPresent()) {
	        ...
	        details.setToss(newdetails.getToss());
	        details.setInnings1Score(newdetails.getInnings1Score());
	        details.setInnings2Score(newdetails.getInnings2Score());
	        details.setWinner(teamRepository.findByTeamid(newdetails.getWinner()).orElse(null)); // Assuming null if team not found
	        details.setManOfTheMatch(playerRepository.findByPlayerId(newdetails.getManOfTheMatch()).orElse(null)); // Assuming null if player not found
